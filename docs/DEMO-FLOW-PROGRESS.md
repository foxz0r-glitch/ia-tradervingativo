...
**‼️ POR QUE O GUARD FOI ARRANCADO (hipótese forte — LEIA ANTES DE MEXER).** `public.user_roles` estava **VAZIA**, nunca teve NENHUM admin. Com ela vazia, `has_role(qualquer, 'admin') = false` → o `/admin` dava `forbidden` pra **TODO MUNDO, inclusive o dono**. Em 09/05 a "solução" foi **tirar a fechadura em vez de criar a chave**. **SE RECOLOCAR O GUARD SEM UM ADMIN NA `user_roles`, O CRM TRANCA PRA TODOS E O CICLO SE REPETE.**

**‼️ DESCOBERTA — o painel admin inteiro provavelmente NUNCA funcionou.** As 4 irmãs (`admin_list_users 20260429205354:79`, `admin_user_stats :101`, `admin_upsert_casatrade 20260429230414:93`, `admin_ops_cross_check 20260620000000:51`) **têm** o guard, e a `user_roles` estava **VAZIA** → todas retornavam `forbidden` pra todo mundo. A **única** função admin que respondia era justamente a que teve o guard arrancado. Isso **reforça** a hipótese da causa-raiz. **⏳ VALIDAR:** com a role criada (17/07), as outras abas do `/admin` (Visão Geral · Gamificação · Aulas · Vendas · Auditoria) devem voltar a funcionar — se voltarem, a hipótese fica provada na prática.
...
- **UI spoofável (código):** `src/hooks/useIsAdmin.ts:12` passa o `_user_id` do **browser** pra `has_role` → um não-admin pode chamar com o uid de um admin e renderizar a **casca** do CRM. O guard novo usa `auth.uid()` (do JWT, **não** spoofável) → os **dados** ficam protegidos, a casca não.
