...
**Dashboard reconstruído = COMPLETO** (4 peças commitadas, validado com print pré-`5cbb489`, idêntico ao handoff). Diferenças restantes do dashboard são só no GRÁFICO (cabeçalho "Euro→Dólar"+bandeiras+"AO VIVO", candlestick vs linha, selo "ROBÔ CONECTADO") → mapeadas pra fatia do gráfico.
...
1. ✅ **FEITO:** Dashboard · gale teto 4 · motor demo (valores) · fundação (fase+overlay+lista própria+cancelamento) · tela Procurando · hero Operando · ativo único + fonte única + lista ao vivo (`85df0f2`) · **loop Modelo B** (`650c781`). fatia 3c (`14fde8b`). **Último commit de código: `64a0b4f`.**
...
4. ✅ **FEITO (`38d4859`, cleanup `8e8672e`): Fatia 4 — Pausado** (gate de pausa D1; booleano demoPaused D2; PAUSAR só no operando D3; novo intervalo ao retomar D4; EM ESPERA/card/⏸ fiéis ao design). Print de runtime PENDENTE (1ª validação visual da pausa) — ver §11.
5. ✅ **FEITO (`b0f47c9`): Fatia 5 — Resultado** (ResultadoScreen fiel ao .dc.html; 2 textos por `demoEndedManually`: "Encerrada manualmente" (PARAR) / "Meta da sessão concluída" (fim natural); rodapé FECHAR→`handleDemoFechar` + DEPOSITAR→`demoDepositRef`; reuso winRate/formatMoeda; keyframes `tv-pop`/`tv-ringExpand` locais). Print de runtime PENDENTE — ver §11. — ✅ **FLUXO DEMO COMPLETO**: 4 telas Procurando (`da8d3df`) → Operando (`40bfabe`/`85df0f2`/`14fde8b`) → Pausado (`38d4859`) → Resultado (`b0f47c9`).
...
10. ⏳ Re-entrância do start da demo (PRÉ-EXISTENTE, fora da Fatia 4): `handleStartDemoSession` depende de `disabled={running}` + `setDemoRunning(true)` síncrono, sem um `startingRef` (que o fluxo real tem). Double-click no mesmo tick poderia iniciar 2 loops (ops/timers duplicados, nunca hang de gate). Blindar quando tocarmos o start da demo; não é regressão.
11. ⏳ **B1 — trigger `handle_new_user` (SQL gated):** o registro (`Auth.tsx`) grava whatsapp SÓ no `user_metadata`; NÃO há trigger sincronizando pra `profiles` → o Admin/CRM (que lê `profiles.whatsapp`) fica cego pro whatsapp de novos cadastros até eles salvarem no /perfil. O port do Perfil (D1) mitiga NO SAVE, não na origem. Fix = trigger `handle_new_user` dando upsert em `profiles` no signup (ou o registro gravar `profiles`). Migration/SQL = gated. Hoje impacto ~0 (pré-lançamento), mas garante perda sistemática de whatsapp no CRM ao lançar.
12. ⏳ **B2 — leitura do telefone no /perfil de `profiles?.whatsapp || meta.whatsapp`** (hoje lê SÓ do `metadata`): o Admin lê de `profiles`; alinhar a leitura (igual ao `savePhone`) fecha o risco (teórico, hoje inalcançável pelos writers do app) de um save do /perfil sobrescrever um `profiles` bom com `metadata` velho, e segue a regra BAR. Próxima fatia; o D1 (gravar nos dois) NÃO mexeu na leitura por decisão do dono.
13. ⏳ **Creds em cleartext (`Index.tsx:923-927`)** — PARQUEADO (⛔ no `CLAUDE.md`; não tocar sem ordem/money-proof).
14. ⏳ **INCIDENTE DE SEGURANÇA — `admin_crm_report()` estava chamável SEM LOGIN** (achado 17/07). Função `SECURITY DEFINER` (bypassa RLS) sem filtro por `auth.uid()` + `proacl` com `PUBLIC`/`anon` → PII de todos os usuários lida com a chave `anon` do bundle público. Sem-login **CONTIDO** (REVOKE manual, já no banco); **guard ⏳ PENDENTE DE APLICAR** na migration `20260717000000_restore_admin_guard_crm_report.sql` (validada por RECON + DOUBLE-CHECK, 0 vermelhos). ‼️ Causa-raiz, contenção, pré-requisitos, achados colaterais e pendências em **§13**.
...
- **ERRATUM conhecido:** `docs/design-handoff/README.md` §9 (L90) chama a linha "procurando" de "última linha"; o RENDER congelado (`Fluxo IA Operando.dc.html` L151-158) a põe como 1º filho do scroll — RENDER vence; snapshot congelado NÃO editado.
...
## 13. 🔴 INCIDENTE DE SEGURANÇA — `admin_crm_report()` chamável sem login (17/07)

> **STATUS:** sem-login **CONTIDO** (REVOKE manual já no banco vivo) · guard **⏳ PENDENTE DE APLICAR** (a migration `20260717000000` foi **committada em `a13a4aa`**; falta **push → Update do Lovable** pra aplicar no banco). **Não marcar ✅ sobre o guard até ele entrar E o /admin carregar.** Item 14 do roadmap (§7) aponta pra cá.

**O QUE ERA (a exposição).** Função `SECURITY DEFINER` (executa com os privilégios do dono → **bypassa RLS**) devolvendo `email`, `whatsapp`, `genero`, `birth_date`, `total_deposited`, `current_balance` e `casatrade_user_id` de **TODOS** os usuários, **sem NENHUM filtro por `auth.uid()`**: não há `WHERE` de nível superior restringindo as linhas ao chamador — a função devolve **uma linha por usuário** de `auth.users` (os únicos `WHERE` do corpo estão dentro dos dois `LATERAL`, correlacionando cada agregado ao próprio `u.id`; ver `20260509120914:71,80` — não são filtro de autorização). Verificado no `prosrc` vivo pelo PRÉ-CHECK 1. `proacl` **MEDIDO** (17/07): `{=X/postgres, anon=X/postgres, ...}` → `PUBLIC` e `anon` com `EXECUTE` → **chamável com a chave `anon` do bundle público do frontend, sem login**. A base tinha **15 usuários** em 17/07 (contador do próprio CRM). ⚠️ **Sobre os valores vazados:** o print de 17/07 mostrou as **11 primeiras** linhas com financeiro zerado (R$ 0,00); as outras **4 não foram vistas** — **não se afirma nada sobre o financeiro dos 15**, nem sobre a janela inteira de ~2 meses. O que é certo: vazou **PII (nome, e-mail, whatsapp) de todos**. Contradiz a RLS de `public.profiles` (`20260430032339:13-22`: "usuários veem o próprio perfil" / "admins veem tudo").

**CAUSA-RAIZ.** A conversão `plpgsql` → `LANGUAGE sql` em `20260509120914:7` **dropou o guard** `IF NOT public.has_role(auth.uid(),'admin') THEN RAISE EXCEPTION 'forbidden'` (`LANGUAGE sql` não comporta guard imperativo) — e o `GRANT EXECUTE ... TO authenticated` (`:86`) **ficou, sem REVOKE**. As **duas versões anteriores da própria função guardavam** (v1 `20260429230414:39`/guard `:58`; v2 `20260430023100:3`/guard `:10`) e **todas as irmãs `admin_*` guardam até hoje** (`admin_list_users 20260429205354:79`, `admin_user_stats :101`, `admin_upsert_casatrade 20260429230414:93`, `admin_ops_cross_check 20260620000000:51`) → **REGRESSÃO, não decisão.**

**‼️ POR QUE O GUARD FOI ARRANCADO (hipótese forte — LEIA ANTES DE MEXER).** `public.user_roles` estava **VAZIA** (MEDIDO 17/07 via SELECT — não derivável do repo; e, pela **ausência de qualquer `INSERT ... user_roles`** em todo o repo, **provavelmente nunca teve admin**). Com ela vazia, `has_role(qualquer, 'admin') = false` → o `/admin` dava `forbidden` pra **TODO MUNDO, inclusive o dono**. Em 09/05 a "solução" foi **tirar a fechadura em vez de criar a chave**. **SE RECOLOCAR O GUARD SEM UM ADMIN NA `user_roles`, O CRM TRANCA PRA TODOS E O CICLO SE REPETE.**

**‼️ DESCOBERTA — o painel admin inteiro provavelmente NUNCA funcionou.** As 4 irmãs (`admin_list_users 20260429205354:79`, `admin_user_stats :101`, `admin_upsert_casatrade 20260429230414:93`, `admin_ops_cross_check 20260620000000:51`) **têm** o guard, e a `user_roles` estava **VAZIA** → **todas ficavam inacessíveis pra todo mundo**: 3 davam `forbidden` (RAISE — `admin_list_users`, `admin_user_stats`, `admin_upsert_casatrade`) e a `admin_ops_cross_check` (padrão `WHERE has_role`, `LANGUAGE sql`) devolvia **lista vazia**. A **única** que devolvia **dados de verdade** era justamente a `admin_crm_report`, que teve o guard arrancado. Isso **reforça** a hipótese da causa-raiz. **⏳ VALIDAR:** com a role criada (17/07), as outras abas do `/admin` (Visão Geral · Gamificação · Aulas · Vendas · Auditoria) devem voltar a funcionar — se voltarem, a hipótese fica provada na prática.

**CONTIDO (manual, 17/07, SQL editor do Lovable — FEITO, já no banco vivo).** `REVOKE EXECUTE ON FUNCTION public.admin_crm_report() FROM PUBLIC;` + `FROM anon;`. `proacl` pós-fix: `{postgres, authenticated, service_role, sandbox_exec}`. **O acesso sem-login acabou.** ‼️ **Aluno logado (`authenticated`) ainda lê tudo até o guard entrar.**

**PRÉ-REQUISITO RESOLVIDO (17/07).** Conta admin `weareboss1337@proton.me` (uid `0250d168-05e9-45bf-86a1-5e363020647f`) criada **pelo SIGNUP DO SITE** + `INSERT` em `public.user_roles` com `role='admin'`. `has_role = true`. O `/admin` abriu pela 1ª vez (15 usuários). **É a ÚNICA conta admin — se ela sumir, o CRM tranca de novo.** Por ter vindo do signup, passou pelo fluxo do `Auth.tsx` (lido 17/07): (1) registro na CasaTrade via `/api/register` **PRIMEIRO** (`Auth.tsx:104`, `PROXY_URL` em `:30`) — como a conta Supabase existe, esse passo **necessariamente teve sucesso** (o `signUp` só roda se a corretora aceita: `:101`, `:136-137`); (2) `signUp` grava no `user_metadata` `{first_name, last_name, country:"Brasil", whatsapp:"+55<dígitos>"}` (`:142`); (3) `insert` em `public.user_credentials` **se** `getSession` devolve sessão (`:152-165`), **SEM checar o erro do insert**. **O signup NÃO grava em `profiles`** (confirma o B1). **PENDENTE-DE-CONFIRMAÇÃO** (só um SELECT prova): se há de fato linha em `user_credentials` pro uid dela — a criação na corretora e o insert podem ter falhado silenciosamente.

**‼️ ACHADO COLATERAL MEDIDO — AUTO-CONFIRM DE E-MAIL.** A conta admin (signup do site, 17/07) teve `created_at 11:58:22.309418` e `email_confirmed_at 11:58:22.345218` — **36 milissegundos**. Nenhum humano confirma em 36ms → o **"Confirm email" do painel está DESLIGADO**. **Resolve o pendente-de-fonte-externa do B1** (o B6 do `RECON-b1-trigger.md`), que estava como "só o painel do Supabase diz". Colateral: o `emailRedirectTo` (`Auth.tsx:141`) fica sem efeito no signup. ⛔ **FATO MEDIDO** — a avaliação da viabilidade das opções do B1 é passo **À PARTE**, depende de código ainda não lido, e **NÃO** é concluída aqui.

**FIX (nesta migration, ⏳ PENDENTE DE APLICAR).** `20260717000000_restore_admin_guard_crm_report.sql`: `CREATE OR REPLACE` (⛔ **ZERO DROP** — o DROP apagaria a ACL e o `CREATE` reconcederia a `PUBLIC`, PIOR que o buraco atual), volta pra `plpgsql` + `IF/RAISE` (forma das irmãs), `#variable_conflict use_column` como defesa, **`SELECT` e assinatura BYTE-IDÊNTICOS** ao `20260509120914` (mesmo `sha256`), + `REVOKE PUBLIC/anon` + `GRANT authenticated`. Validado por `RECON-guard-crm.md` + `DOUBLE-CHECK-…-guard-crm-2026-07-17.md` (**0 vermelhos, 5 lentes**).

**‼️ VALIDAÇÃO OBRIGATÓRIA PÓS-APLICAÇÃO.** `plpgsql` **adia a checagem do corpo pra 1ª CHAMADA** — a migration pode "aplicar com sucesso" e o CRM quebrar. A **única** prova é **abrir o `/admin` e ver a lista carregar**. **Rollback:** reaplicar `20260509120914:7-84` **SEM** o DROP da L5.

**⏳ PENDÊNCIAS (itens separados).**
- **UI spoofável (código):** `src/hooks/useIsAdmin.ts:12-13` passa o `_user_id` do **browser** (`session.user.id`) pra `has_role` → um não-admin pode chamar com o uid de um admin e renderizar a **casca** do CRM. O guard novo usa `auth.uid()` (do JWT, **não** spoofável) → os **dados** ficam protegidos, a casca não.
- **Resposta a incidente:** **NÃO** foi verificado se a RPC foi chamada por `anon` nos ~2 meses de exposição (09/05 → 17/07). Os **Logs** do painel dirão. Muda a natureza do caso (incidente **contido** vs vazamento **consumado**).
- **Armazenamento de credencial (PARQUEADO — ⛔ não tocar sem ordem/money-proof).** `public.user_credentials` tem `casatrade_password` e `casatrade_ssid` como colunas **`text`** (schema medido 17/07 via `information_schema`; a coluna que referencia o usuário é **`id`**, não `user_id`). ‼️ **O TIPO `text` NÃO PROVA o conteúdo** — uma coluna `text` pode guardar valor cifrado. **INDÍCIO** de que está em claro: `Auth.tsx:161` grava `casatrade_password: password` (o valor do input direto de `:87`, **sem nenhum passo de cifragem/encode** antes do insert); o app faz `console.log` de `{email, password, ssid}` (`Index.tsx:923-927`, parqueado) e envia as credenciais como JSON no WS (`useRoboBot.ts:80-82`); **nenhum passo de decrypt foi encontrado**. **PENDENTE-DE-CONFIRMAÇÃO** — só um SELECT no valor provaria, e isso **não foi feito de propósito** (não expor segredo). Mesma classe do `console.log` parqueado (§7 item 13), mas na camada de **armazenamento**.

**CRUZA COM O B1.** A opção (d) do B1 (`COALESCE(pr.whatsapp, metadata)` no leitor) **AMPLIARIA** esta função — **este guard era pré-requisito daquela decisão.**
