/**
 * Ranking 14 — clone visual do Ranking11 com variação ÚNICA na faixa
 * entre o título "Temporada · …" e o input "Buscar trader".
 *
 * Variante desta página: TIMELINE / CHECKPOINTS — barra com marcos e pin "você está aqui"
 *
...
import UserBadge from "./ranking/_UserBadge14";
...
export default function Ranking14() {
  ...
  return (
    <div className="relative min-h-screen px-4 py-10 md:px-8">
      ...
      <div className="mx-auto max-w-6xl">
        {/* JORNADA — header tipo "rota da temporada" com pin "você está aqui" */}
          ...
          >
            ...
            <div className="relative px-6 pb-6 pt-7 md:px-10">
              ...
              <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  <span className="rounded-md border border-amber-400/40 bg-amber-400/10 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.28em] text-amber-300 shadow-[0_0_12px_-4px_rgba(251,191,36,0.7)]">
                    LAP {String(season.day).padStart(2, "0")}/{String(season.total).padStart(2, "0")}
                  </span>
                  <span className="rounded-md border border-border/60 bg-card/40 px-3 py-1 text-[12px] font-bold uppercase tracking-[0.22em] text-muted-foreground">
              ...
              {/* Trilha da jornada */}
              <div className="relative mt-7 px-2 pb-1 pt-7">
                <div className="relative h-2 rounded-full bg-[hsl(220_22%_12%)]">
                  <div
                  ...
                  {[
                    ...
                    { p: 25, label: "1ª Etapa" },
                    { p: 50, label: "Meio" },
                    { p: 75, label: "Reta Final" },
                    { p: 100, label: "Final" },
                  ].map((cp) => {
                    const reached = season.pct >= cp.p;
                    return (
                      <div key={cp.p} className="absolute top-1/2 -translate-x-1/2 -translate-y-1/2" style={{ left: `${cp.p}%` }}>
                        <div
                  ...
                  <div
                    ...
                    style={{ left: `${Math.min(98, Math.max(2, season.pct))}%` }}
                  >
                    <div className="flex flex-col items-center">
                      <div className="rounded-md border border-amber-400/60 bg-amber-400/15 px-2 py-0.5 font-mono text-[9px] font-black uppercase tracking-widest text-amber-300 shadow-[0_0_14px_-2px_rgba(251,191,36,0.85)]">
                        VOCÊ · {Math.round(season.pct)}%
                      </div>
                      <div className="h-2 w-px" style={{ background: "rgba(251,191,36,0.7)" }} />
                    </div>
                  </div>
                </div>
              </div>
