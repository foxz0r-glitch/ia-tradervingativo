/**
 * Ranking 12 — clone visual do Ranking11 com variação ÚNICA na faixa
 * entre o título "Temporada · …" e o input "Buscar trader".
 *
 * Variante desta página: CIRCULAR DASHBOARD — anel de progresso central + cards laterais
 *
...
export default function Ranking12() {
            ...
            <div className="relative grid items-center gap-6 px-6 py-7 md:grid-cols-[1fr_auto_1fr]">
              ...
              <div className="relative mx-auto flex h-[200px] w-[200px] items-center justify-center">
                <svg viewBox="0 0 200 200" className="h-full w-full -rotate-90">
                  ...
                  {/* trilho */}
                  <circle cx="100" cy="100" r="84" fill="none" stroke="hsl(220 22% 14%)" strokeWidth="14" />
                  {/* progresso */}
                  <circle
                    cx="100"
                    cy="100"
                    ...
                    strokeDashoffset={`${2 * Math.PI * 84 * (1 - season.pct / 100)}`}
                    filter="url(#r12-glow)"
              ...
              <div className="flex flex-col items-center gap-3 md:items-start">
                <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.32em] text-amber-300">
                  <Timer className="h-3 w-3" /> Lap
                </div>
                <div className="flex items-baseline gap-1 font-black tabular-nums text-foreground" style={{ fontSize: 44, lineHeight: 1, letterSpacing: "0.02em" }}>
                  {String(season.day).padStart(2, "0")}
                  <span className="text-amber-300/60 text-3xl">/</span>
                  <span className="text-amber-300/80 text-3xl">{String(season.total).padStart(2, "0")}</span>
                </div>
                <div className="text-[10px] font-semibold uppercase tracking-[0.22em] text-muted-foreground">
                  Dia {season.day} do mês
                </div>
              </div>
            </div>
          </div>
