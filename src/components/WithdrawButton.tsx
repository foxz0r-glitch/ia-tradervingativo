...
import { ArrowUpRight, Copy, Check, Mail, Lock, ArrowRight, Eye, EyeOff } from "lucide-react";
...
export const WithdrawButton = () => {
  ...
  const WITHDRAW_URL = "https://trade.casatrade.com/pt/withdrawal";

  const handleSacar = () => {
    // Abrimos o diálogo de forma síncrona (gesto do usuário) e
    // carregamos as credenciais em segundo plano.
    setOpen(true);
    (async () => {
      try {
  ...
  return (
    <>
      <button
        ...
        aria-label="Sacar"
        title="Sacar"
        className="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-border/70 bg-secondary/40 text-foreground/80 transition-all hover:border-primary/40 hover:bg-secondary hover:text-foreground active:translate-y-px"
      >
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="overflow-hidden border-0 bg-transparent p-0 shadow-none sm:max-w-md">
          <DialogTitle className="sr-only">Sacar da corretora</DialogTitle>
          <DialogDescription className="sr-only">
            Use seus dados de acesso para entrar na corretora e realizar o saque.
          </DialogDescription>
          {/* Glass card */}
